...
module Mutations
  class Predicates::SearchPredicate < Mutations::BaseMutation
    ...
    def app_users(per, page)
      ...
      .where.not(email: nil, subscription_state: "archived")
    ...
    def resolve(app_key:, search:, page:, per:)
      ...
      resource_params = search.require(:data).permit(:source, :source_id, predicates: %w[attribute comparison type value])
