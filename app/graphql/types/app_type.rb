...
module Types
  class AppType < Types::BaseObject
    ...
    field :stripe_plan_id, String, null: true
    field :available_plans, [Types::PlanType], null: true
    field :plan, Types::PlanType, null: true
    field :billing, Types::JsonType, null: true
    field :email_forwarding_address, String, null: true

    def stripe_plan_id
      object.stripe_plan_id.blank? ? "free-plan" : object.stripe_plan_id
    end
    ...
    def article_settings
      if  object.article_settings.blank?
        default_url = object.name.gsub(" ","_").underscore
        if ArticleSetting.where(domain: default_url).count > 0
          default_url = default_url + object.id.to_s
        end
          object.create_article_settings(domain: default_url)
      else
        object.article_settings
      end
    end
    ...
    field :upgrade_for_api_integrations, Boolean, null: false
    def upgrade_for_api_integrations
      object.feature_needs_upgrade('upgrade_for_api_integrations')
    end

    field :upgrade_to_add_webhooks, Boolean, null: false
    def upgrade_to_add_webhooks
      object.feature_needs_upgrade('upgrade_to_add_webhooks') 
    end

    field :upgrade_to_add_agents, Boolean, null: false
    def upgrade_to_add_agents 
      object.feature_needs_upgrade('upgrade_to_add_agents')  
    end

    field :upgrade_to_create_campaigns, Boolean, null: false
    def upgrade_to_create_campaigns 
      object.feature_needs_upgrade('upgrade_to_create_campaigns') 
    end

    field :upgrade_to_create_bot_tasks, Boolean, null: false
    def upgrade_to_create_bot_tasks 
      object.feature_needs_upgrade('upgrade_to_create_bot_tasks') 
    end
