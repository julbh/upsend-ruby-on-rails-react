...
class AppUser < ApplicationRecord
  ...
  after_create :add_created_event, unless: :skip_callbacks
  after_save :enqueue_social_enrichment, :if => Proc.new{ saved_change_to_email? && !skip_callbacks }
  ...
  scope :campaign_audience, lambda { |campaign_id|
    joins("left outer join metrics
     on metrics.trackable_type = 'Message'
     AND metrics.trackable_id = #{campaign_id}
     AND metrics.app_user_id = app_users.id"
    ).where("metrics.id is null")
  }


  scope :unarchived, -> { where.not(subscription_state: 'archived') }
  ...
  aasm column: :subscription_state do # default column: aasm_state
    ...
    event :subscribe do
      transitions from: %i[passive unsubscribed archived blocked bounced complained], to: :subscribed
    end
    ...
    event :bounce do
      transitions from: %i[subscribed passive archived blocked unsubscribed], to: :bounced
    end

    event :complaint do
      transitions from: %i[subscribed passive archived blocked unsubscribed], to: :complained
    end
  ...
  def inactive_subscription?
    %w[unsubscribed complained bounced].include?(subscription_state)
  end
  ...
  # def assign_user_data
  #   if signed_up.blank? && properties["signed_up"].present?
  #     self.signed_up = properties["signed_up"]
  #   end
  # end 
