...
module StripePlanMethods
  ...
  def free_plan? 
    stripe_plan_id == "free-plan" ? true : false 
  end
  
  def pro_plan? 
    stripe_plan_id == "standard-monthly"  ? true : false 
  end

  def unlimited_plan?
    stripe_plan_id == "premium-monthly" ? true : false 
  end

  def agent_limit_exhausted?
    total_agents = agents.size
    quota = active_plan.seats
    return false if quota == "unlimited"
    max_limit = active_plan.seats.to_i
    can_add_agents = (total_agents < max_limit) ? true : false   
    can_add_agents ? false : true
  end
  ...
  def active_plan
    paid_plan_user = plan && ['pro','unlimited'].include?(plan.name.downcase)
    if plan && paid_plan_user   
      plan
    elsif !trial_expired?
      Plan.where(stripe_plan_id: "standard-monthly").first
    else
  ...
  def trial_expired? 
    if (trial_period && !trial_period.active) || self.trial_remaining_days <= 0
      return true
    else
      return false
    end
        ...
        if selected_plan.amount_cents > 0
          ...
          if result
            ...
            if result.status == "active" || result.status == "trialing"
              self.update_attributes(stripe_plan_id: stripe_plan_id)
              current_subscription.update_attributes!(:status => "Upgraded to #{ selected_plan.name }") if !current_subscription.blank?
            elsif !current_subscription.blank?
              current_subscription.update_attributes!(:status => "(Previous Status: #{current_subscription.status}) - Auto Disabled in case of PAST_DUE: waiting on payment confirmation for #{ selected_plan.name }") 
            end
