...
class Campaign < Message
  ...
  enum template: [:plain, :company, :personal]
  ...
  def self.scheduled_newsletters_job
    Campaign.scheduled.in_time.each do |campaign|
  ...
  def test_newsletter(to_addresses, sender, opts)
    CampaignMailer.test(self,to_addresses, sender, opts).deliver_later
  end
  ...
  def apply_premailer(opts = {})
    ...
    premailer_url = ["#{host}/apps/#{app.key}/premailer/#{id}", skip_track_image].join("?sender_id=#{opts[:sender]&.id}&template=#{opts[:template]}&include_unsubscription_link=#{opts[:include_unsubscription_link]}&footer_address=#{opts[:footer_address]}&help_email=#{opts[:help_email]}&help_contact=#{opts[:help_contact]}&")
