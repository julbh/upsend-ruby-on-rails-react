...
class App < ApplicationRecord
  ...
  def default_custom_fields
    #To prevent duplicates in UI. This columns should not go into app_user properties:{}
    AppUser.column_names
  end 

  def add_custom_fields_from(import_contact) 
    available_fields = default_custom_fields + (custom_fields ? custom_fields.map{|x| x[:name]} : [])
    user_fields = import_contact.mapping_fields.map{|x| x["value"]}.uniq.compact.reject{|x| x.blank?}
    new_fields = user_fields - available_fields 
    self.custom_fields = [] if custom_fields.blank?
    new_fields && new_fields.each do |key|
      self.custom_fields << {"name"=> key, "type"=>"string"}
    end
    self.save if new_fields
  end

  def feature_needs_upgrade(action)
    return false if Rails.env.development?
    

    case action
    when "upgrade_for_api_integrations"
      free_plan? ? true : false
    when "upgrade_to_add_webhooks"
      free_plan? ? true : false
    when "upgrade_to_add_agents"
      agent_limit_exhausted?
    when "upgrade_to_create_campaigns"
      free_plan? ? true : false
    when "upgrade_to_create_bot_tasks"
      free_plan? ? true : false
    else
      free_plan? ? true : false
    end
