...
class DanteTooltip extends React.Component {
  ...
  blockAlignmentChange =(e, alignmentType)=> {
    e.preventDefault()
    let { editorState } = this.props
    //let urlValue = e.currentTarget.value
    let selection = editorState.getSelection()
    ...
    let entityKey = Entity.create('block-alignment', 'MUTABLE', opts)
    //contentState.createEntity('LINK', 'MUTABLE', opts)
          ...
          { this.props.widget_options.block_types.map( (item, i) => {
              switch (item.type) {
                ...
                case "block-alignment":
                  return <DanteTooltipTextCenter
                            key={ i }
                            editorState={ this.props.editorState }
                            blockAlignmentChange={ this.blockAlignmentChange }
                            alignmentType={item.style}
                          />
                  break;  
                default:
                  break;
...
class DanteTooltipTextCenter extends React.Component {

  constructor(...args) {
    super(...args)
    this.blockAlignmentChange = this.blockAlignmentChange.bind(this)
  }

  blockAlignmentChange =(ev)=> {
    let selection = this.props.editorState.getSelection()
    //if (!selection.isCollapsed()) {
      return this.props.blockAlignmentChange(ev, this.props.alignmentType)
    //}
  }
  ...
  render() {
    ...
    return (
      <li className="dante-menu-button" onMouseDown={ this.blockAlignmentChange }>
        <span className={ 'dante-icon'}>
