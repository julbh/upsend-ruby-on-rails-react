...
function SimpleListMenu(props) {
  ...
  function getAppUsers(){
    graphql(APPS ,{} ,{
      success: (data)=>{
        setState({apps: data.apps, show_submenu: true})
      }, 
      error: (error)=>{
        console.log(error)
      }
  ...
  return (
    <React.Fragment>
      ...
      {state.show_submenu === true ? 
        <Menu className="avatar-submenu" id="lock-menu" anchorEl={anchorEl}
          open={Boolean(anchorEl)} 
          onClose={handleClose}
          transformOrigin={{
            vertical: 'top',
