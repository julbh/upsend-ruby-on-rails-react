...
export const helpCenterUrl = (article_setting) => {
  return (
    article_setting ? 
      ( article_setting.subdomain ? article_setting.subdomain : `helpcenter.${process.env.DOMAIN}/` + article_setting.domain + "/en" )
    :
    null 
  )
}
...
class Settings extends Component {
  ...
  definitionsForSettings = () => {
    return [
      {
        name: "domain",
        hint: "documentation site default domain",
        type: 'string',
        grid: { xs: 12, sm: 12 },
        startAdornment: `helpcenter.${process.env.DOMAIN}/`
      },
      {
        name: "subdomain",
       ...
       <React.Fragment>
         ...
         <ContentHeader 
            ...
            items={
              <React.Fragment>
                {
                  helpCenterUrl(this.props.settings) ?
                    <Grid item>
                      <Button href={`http://${helpCenterUrl(this.props.settings)}`}
