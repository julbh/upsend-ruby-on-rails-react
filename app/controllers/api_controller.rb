...
class ApiController < ActionController::API
  ...
  def get_user_from_unencrypted
    ...
    if user_data.present?
      ...
      is_valid = validate_user_data(json_response) 
      resp = is_valid ? {email: json_response[:email], properties: json_response} : nil 
    end 
    resp.present? ? resp : {}
  rescue StandardError
    nil
  ...
  def validate_user_data(json_response) 
    return false if (json_response[:email] =~ URI::MailTo::EMAIL_REGEXP).nil? || json_response[:name].downcase.include?("user_name (your attributes)")
    return true
  end
