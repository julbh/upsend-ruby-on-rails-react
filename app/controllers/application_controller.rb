...
class ApplicationController < ActionController::Base
  ...
  def preview
    ...
    @sender = Agent.find_by_id(params[:sender_id])
    # render plain: "hello"
    if template = (params[:template].presence || @campaign.template)
      render "campaigns/mail_template/#{template}", layout: false
    else
  ...
  def current_resource_owner
    Agent.find(doorkeeper_token.resource_owner_id) if doorkeeper_token && doorkeeper_token.resource_owner_id
  end
